...
Color plots, condensation (nog aan verder werken)
...
for i in range(len(design_grid)):       
        ...
        output_filename = folder + '/MW1.out' 

        if os.path.exists(output_filename)==True:
           
            if design_grid['grid'][i]==0:
                data=pd.read_csv(output_filename,skiprows=range(3896), header=None,sep='\s+')
            if design_grid['grid'][i]==1:
                data=pd.read_csv(output_filename,skiprows=range(4012), header=None,sep='\s+')
            if design_grid['grid'][i]==2:
                data=pd.read_csv(output_filename,skiprows=range(4361), header=None,sep='\s+')
            if design_grid['grid'][i]==3:
                data=pd.read_csv(output_filename,skiprows=range(8420), header=None,sep='\s+')
            if design_grid['grid'][i]==4:
                data=pd.read_csv(output_filename,skiprows=range(8536), header=None,sep='\s+')
            if design_grid['grid'][i]==5:
                data=pd.read_csv(output_filename,skiprows=range(8884), header=None,sep='\s+')                
                
            
            data['condens'] = data.sum(axis=1)-data[0]
            data['condens'] =  data['condens']*5.26/1000/2.4   #kg/m² uitgemiddeld
            
            condens=pd.DataFrame(data['condens'])            
            
            max_MC.append(condens.max())
            del data        
...
c_un_MRC0=np.array(c_MRC0)
c_un_MRC1=np.array(c_MRC1)
c_un_MRC2=np.array(c_MRC2)
c_un_MRC3=np.array(c_MRC3)
...
v = np.linspace(0.01, 10, 50, endpoint=True)
...
fig1.savefig('C:/Users/jelle/Desktop/plots/contour_grid5_t.png',dpi=400)
